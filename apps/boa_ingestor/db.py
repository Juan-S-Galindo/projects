"""Database setup for the Bank of America checking account ingestor.
...
On startup:
  ...
  2. Connects to the target database and creates the `staging` schema and
     the `staging.bank_of_america_transactions` table if they do not already
     exist.
"""
...
CREATE_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS staging.bank_of_america_transactions (
...
def ensure_schema_and_table() -> None:
    """Create the `staging` schema and `bank_of_america_transactions` table if
    absent."""
